...
import { automationNames } from './automation-names';
...
class BaseDriver extends Protocol {
  ...
  memoizeFunctions () {
    // there are functions we want to memoize, but it needs to be done on
    // each instance, not on the class
    if (this._memoizationDone) {
      // only do this process once
      return;
    }

    for (const fn of this.getFunctionsToMemoize()) {
      if (!_.isFunction(this[fn])) {
        log.warn(`Unable to memoize function '${fn}'`);
        continue;
      }
      this[fn] = _.memoize(this[fn]);
    }
    this._memoizationDone = true;
  }

  getFunctionsToMemoize () {
    // functions to be memoized on each instance of the class
